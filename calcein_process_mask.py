# python calcein_process_mask.py plate_no treatments_file
...
os.system('mkdir processed/plate' + p + '/calcein_cells')
for g in treatments.guide.unique():
    os.system('mkdir processed/plate' + p + '/calcein_cells/' + g)
...
def process_mask(dic, well):
    ...
    dapi = cv2.imread(dic[well][3], cv2.IMREAD_GRAYSCALE)
    ...
    mask = plt.imread('calcein_masks/plate' + pl + 'mask_' + well + '.tif')
    r = 60
    d = mask.shape[0]
            ...
            if numpy.max(m)>0:
                ...
                if (r < centroid[0] < d-r) & (r < centroid[1] < d-r):
                    ...
                    if numpy.max(mcell) > 0:
                        ...
                        numpy.save('processed/plate' + pl + 'calcein_cells/' + treatments[treatments.well == well[:3]].guide.item() + '/' + well + '_' + str(x), stcell)
                        skimage.io.imsave('processed/plate' + pl + 'calcein_cells/' + treatments[treatments.well == well[:3]].guide.item() + '/' + well + '_' + str(x) + '.tif', stcell[:,:,:3])
